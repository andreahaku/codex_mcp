...
name: codex
description: >
  Local Codex CLI (OpenAI GPT-6 Sol) for second opinions, code review, iterative collaboration.
...
Use the local `codex` CLI directly (GPT-6 Sol default, high; `--fast` = GPT-6 Luna, never for reviews). Do not use this repository's MCP server when this skill is active.
